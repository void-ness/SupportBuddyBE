...
class BatchProcessor:
    ...
    async def process_inactive_user_reminders_in_batches(self, batch_size: int = 5):
        logger.info("Starting batch processing for Notion users.")
        active_notion_users = await self.user_manager.get_recently_active_notion_users()
        logger.info(f"Found {len(active_notion_users)} recently active Notion users.")

        if not active_notion_users:
            logger.info("No recently active Notion users to process.")
            return
        ...
        for i in range(0, len(active_notion_users), batch_size):
            ...
            for user in batch:
                logger.info(f"Adding task for user {user.id} ({user.email})")
                tasks.append(self.journal_manager.send_reminder_email(user))
